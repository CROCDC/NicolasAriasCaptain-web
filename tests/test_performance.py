...
# Same marker the rest of the browser suite carries: these drive a real
# Chromium and take seconds, so `make test-fast` leaves them out.
...
# Weight & request budgets are measured on a COLD load (empty cache), so they
# are deterministic and worth setting close: these are calibrated to what this
# site actually ships (282KB over 13 requests) plus room to grow, not to a
# generic ceiling that would let the page double before anything complained.
PAGE_WEIGHT_BUDGET_KB = 420    # measured 282
JS_WEIGHT_BUDGET_KB = 60       # measured 4 — one small deferred script
CSS_WEIGHT_BUDGET_KB = 60      # measured 27 — critical inline + deferred sheet
IMAGE_WEIGHT_BUDGET_KB = 250   # measured 143 — the responsive variants, not the originals
FONT_WEIGHT_BUDGET_KB = 150    # measured 93 — the latin cuts of three families

REQUEST_COUNT_BUDGET = 20      # measured 13
DOM_NODES_BUDGET = 1000        # measured 741
JS_HEAP_BUDGET_MB = 20         # measured 10
...
def test_no_excessive_long_tasks(warm_samples: list[dict]) -> None:
    ...
    assert worst < MAX_LONG_TASK_MS, (
        f"Longest main-thread task {worst:.0f}ms exceeds {MAX_LONG_TASK_MS}ms "
        f"(blocks interactivity)"
    )
...
def test_javascript_weight(cold_metrics: dict) -> None:
    ...
    assert js < JS_WEIGHT_BUDGET_KB, (
        f"JS weighs {js:.0f}KB, exceeds the budget of {JS_WEIGHT_BUDGET_KB}KB")


def test_css_weight(cold_metrics: dict) -> None:
    css = cold_metrics["byType"].get("css", 0.0)
    assert css < CSS_WEIGHT_BUDGET_KB, (
        f"CSS weighs {css:.0f}KB, exceeds the budget of {CSS_WEIGHT_BUDGET_KB}KB")
...
def test_render_blocking_stylesheets_within_budget(browser: Browser, live_server: str) -> None:
    ...
    assert len(blocking) <= RENDER_BLOCKING_CSS_BUDGET, (
        f"{len(blocking)} render-blocking stylesheets, exceeds "
        f"{RENDER_BLOCKING_CSS_BUDGET}: {blocking}"
    )
...
def test_limited_third_party_origins(browser: Browser, live_server: str) -> None:
    ...
    assert len(origins) <= THIRD_PARTY_ORIGINS_BUDGET, (
        f"{len(origins)} third-party origins, exceeds "
        f"{THIRD_PARTY_ORIGINS_BUDGET}: {sorted(origins)}"
    )
